...
interface PersonaFeedbackProps {
  ...
  persona: string;
  onAccept: (persona: string, feedback: FeedbackItem) => void;
  onReject: (persona: string, feedback: FeedbackItem) => void;
}

interface AcceptedRejectedFeedback {
  persona: string;
  feedback: FeedbackItem;
}

const PersonaFeedback: React.FC<PersonaFeedbackProps> = ({ feedback, persona, onAccept, onReject }) => {
  return (
    <Card className="mb-4">
...
const PRDAnalyzer: React.FC = () => {
  const [prdText, setPrdText] = useState<string>('');
  const [isAnalyzing, setIsAnalyzing] = useState<boolean>(false);
  const [feedbackData, setFeedbackData] = useState<FeedbackData | null>(null);
  const [orchestratorFeedback, setOrchestratorFeedback] = useState<string>('');
  const [acceptedFeedback, setAcceptedFeedback] = useState<AcceptedRejectedFeedback[]>([]);
  const [rejectedFeedback, setRejectedFeedback] = useState<AcceptedRejectedFeedback[]>([]);

  const analyzePRD = async () => {
    setIsAnalyzing(true);
    try {
      // Mock API call - replace with actual backend call
      const mockResponse: FeedbackData = {
        product_manager: {
          key_insights: [
  ...
  const handleAcceptFeedback = (persona: string, feedback: FeedbackItem) => {
    setAcceptedFeedback([...acceptedFeedback, { persona, feedback }]);
    setRejectedFeedback(rejectedFeedback.filter(f => f.persona !== persona));
  };

  const handleRejectFeedback = (persona: string, feedback: FeedbackItem) => {
